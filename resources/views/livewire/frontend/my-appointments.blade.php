                        ...
                        <div class="sm:col-span-1">
                            ...
                            <div class="relative">
                                <input type="text" wire:model.live.debounce.300ms="search"
                                    id="hs-as-table-product-review-search" name="search"
                                ...
                                @foreach ($all_appointments as $appointment)
                                    <tr class="bg-white hover:bg-gray-50 dark:bg-neutral-900 dark:hover:bg-neutral-800">
                                        <td class="size-px whitespace-nowrap">
                                            <div class="ps-6 lg:ps-3 xl:ps-0 pe-6 py-3">
                                                <div class="flex items-center gap-x-3">
                                                    <livewire:frontend.profile-image :id="$appointment->doctor->id" />
                                                    <div class="grow">
                                                        <span
                                                            class="block text-sm font-semibold text-gray-800 dark:text-neutral-200">{{ $appointment->doctor->user->name }}</span>
                                                        <span
                                                            class="block text-sm text-gray-500 dark:text-neutral-500">{{ $appointment->doctor->user->email }}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="size-px whitespace-nowrap align-top">
                                            <a class="block p-6">
                                                <span
                                                    class="text-sm text-gray-600 dark:text-neutral-400">{{ $appointment->appointment_type == 'onsite' ? 'On site' : 'Live Consultation' }}</span>
                                            </a>
                                        </td>
                                        <td class="size-px whitespace-nowrap align-top">
                                            <a class="block p-6">
                                                <span
                                                    class="text-sm text-gray-600 dark:text-neutral-400">{{ date('d M Y', strtotime($appointment->appointment_date)) }}</span>
                                            </a>
                                        </td>
                                        <td class="size-px whitespace-nowrap align-top">
                                            <a class="block p-6">
                                                <span
                                                    class="text-sm text-gray-600 dark:text-neutral-400">{{ date('H:i A', strtotime($appointment->appointment_time)) }}</span>
                                            </a>
                                        </td>
                                        <td class="size-px whitespace-nowrap align-top">
                                            <div class="block p-6">
                                  ...
                                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
                                      <path stroke-linecap="round" stroke-linejoin="round" d="m15.75 10.5 4.72-4.72a.75.75 0 0 1 1.28.53v11.38a.75.75 0 0 1-1.28.53l-4.72-4.72M4.5 18.75h9a2.25 2.25 0 0 0 2.25-2.25v-9a2.25 2.25 0 0 0-2.25-2.25h-9A2.25 2.25 0 0 0 2.25 7.5v9a2.25 2.25 0 0 0 2.25 2.25Z" />
                                                ...
                                                <a href="{{ $appointment->status == 'complete' ? '#' : '/reschedule/' . $appointment->id . '' }}"
                                                ...
                                                <button class="bg-red-500 rounded text-white p-1 ml-3"
                                                    {{ $appointment->status == 'complete' ? 'disabled' : '' }}
                                                    wire:click="cancel({{ $appointment->id }})"
                                                    wire:confirm="Are you really want to cancel the appointment?">
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none"
                                                        viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
                                                        class="size-6">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
